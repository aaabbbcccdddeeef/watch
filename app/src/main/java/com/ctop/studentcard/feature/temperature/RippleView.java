...
public class RippleView extends View {
    ...
    private void drawInCircle(Canvas canvas) {
        ...
        // 处理每个圆的宽度和透明度
        mPaint.setAlpha(mCircle.alpha);// （透明）0~255（不透明）
        canvas.drawCircle(mWidth / 2, mHeight / 2, mCircle.width - mPaint.getStrokeWidth(), mPaint);

        if (mCircle.width <= mRadius) {
            // 计算不透明的数值，这里有个小知识，就是如果不加上double的话，255除以一个任意比它大的数都将是0
            if (mIsAlpha) {
                double alpha = 255 - mCircle.width * (255 / ((double) mWidth / 2));
...
//    }
    ...
    public void setRadius(float radius, Handler handler) {
        mRadius = radius;
        mHandler = handler;
    }
